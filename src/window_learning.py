...
def fractional_learning(init):
    '''
    Implementation of a learnable fraction orders function with
    parameter with a uniform itinialiation.

    Returns:
              learned fraction order frac
    '''
    with tf.variable_scope("fraction_order"):
        init = tf.constant(init,dtype=tf.float32)
      
        frac = tf.get_variable('frac', dtype=tf.float32,
                   initializer=init,
                 #       constraint=lambda t: tf.clip_by_value(t, -2,2), # You can push values to be between -2 and 2 or any interval you desire
                                    trainable=True)
         
    
        tf.summary.scalar('fraction', frac)
        return frac
