...
''''
Implemetation of the fractiona Fourier transform matrix with respect to learnable parameter a 
Input: fraction order a and size of the input to be transformed of size N
output: NxN complex-valued matrix
'''
def dfrtmtrx(N, a):
    # Approximation order
    app_ord = 2
    Evec =  tf.convert_to_tensor(_dis_s(N,app_ord))
    Evec= tf.cast(Evec,tf.complex64)
    even = 1 - (N%2)
    
    l =  tf.convert_to_tensor(np.array(list(range(0,N-1)) + [N-1+even]))
    l= tf.cast(l,tf.complex64)
    
    
    f = tf.linalg.diag(tf.math.exp(-1j*math.pi/2*tf.cast(a,tf.complex64)*l))
    F= N**(1/2)*tf.einsum("ij,jk,ni->nk", f, tf.transpose(Evec), Evec)
