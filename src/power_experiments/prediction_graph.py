...
def compute_parameter_total(trainable_variables):
    ...
    print("Last parameter name is", variable.name)
...
class FFTpredictionGraph(object):
    ...
    def __init__(self, pd, generator=None):
        ...
        with self.graph.as_default():
            ...
            fraction= wl.fractional_learning(pd["fraction"])  # if you don't want the fraction parameter as learnable, just go the "wl>>fractional_learning and change as trainable=False"
            ...
            if generator:
                ...
                print('-------------------- DATA SHAPE IS ---------------------------- ',data_nd_norm.shape)
                ...
                print("----------------------- DATA SHAPE IS---------------------",data_nd.shape)
            ...
            if pd['fft']:
                ...
                def transpose_stft_squeeze(in_data, window, pd):
                    ...
                    in_data_fft = eagerSTFT.stft(tmp_in_data, window,
                                                 pd['window_size'], pd['overlap'],frac=fraction)#frac=pd['fraction'])
            ...
            if pd['cell_type'] == 'cgRNN':
                ...
                # gru = rnn_cell.BasicRNNCell(pd['num_units'])
                if pd['fft'] is True:
                    dtype = tf.float32
            ...
            time_loss =  tf.compat.v1.losses.mean_squared_error (
            ...
            learning_rate = tf.compat.v1.train.exponential_decay(pd['init_learning_rate'],
                                                       global_step,
                                                       pd['decay_steps'],
                                                       pd['decay_rate'],
                                                       staircase=True)
            ...
            # optimizer= tf.compat.v1.train.ROptimizer(learning_rate=learning_rate) 
            ...
            # Prints the name of parameter to take gradient, you can see that fraction parameter is in this list
            with open("file3.txt", "w") as output:
                      capped_gvss = [var.name for grad, var in gvs]
                      output.write(str(capped_gvss))
            
            self.training_op = optimizer.apply_gradients(capped_gvs,  global_step=global_step)
            ...
            print("Total Parameter:", compute_parameter_total(tf.trainable_variables()))
