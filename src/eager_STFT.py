...
''''
dfrt_3D(data,a): takes a 4D data [batch_size, #of windows, window length, 1] and fraction order a, and initially convert data to 3D by squeezing in the last dimension. Then 
on the last dimension, creates the FrFT matrix to use 
...
def dfrt_3D(data,a):
     ...
     bs,wn,sp = data.shape[0].value,data.shape[1].value,data.shape[2].value
     
   
     dfrt_tensor = tf.convert_to_tensor(dfrt.dfrtmtrx(sp,a))
     
     return bs,wn,sp,dfrt_tensor,data
...
def stft(data, window, nperseg, noverlap, frac=1.0, nfft=None, sides=None, padded=True,
    ...
    with tf.variable_scope("stft"):
        ...
        print( "-------------------------------------Batched data shape in time is -------------------------------  ", result.shape)
...
####################################################################################################################
        '''
           Use the function dfrt_3D to create FrFT matrix and transform data using this matrix
        '''        
        bs,wn,sp,dfrt_tensor,data = dfrt_3D(result1,frac)
        
        data= tf.cast(data,tf.complex64)
        data =tf.einsum('ij,klj->kli',dfrt_tensor,data)
        data = tf.reshape(data,[bs,1,wn,sp])
        result= data
...
####################################################################################################################      
        ...
        print(' -----------------------------FFT is taken successfully-----------------------------------------------')


        print( " --------------------------------- Data shape after FFT in freq is ------------------------ ", result.shape)
...
def istft(Zxx, window, nperseg=None, noverlap=None,frac=1.0,nfft=None,
...
####################################################################################################################
        '''
           Inverse FrFT is taken here
        '''
        bs,wn,sp,dfrt_tensor,data = dfrt_3D(Zxx,-frac)
        
        data= tf.cast(data,tf.complex64)
        data =tf.einsum('ij,klj->kli',dfrt_tensor,data)/sp
        data = tf.reshape(data,[bs,1,wn,sp])
        result= data
...
####################################################################################################################
        ...
        print('------------------------------------------------------ Inverted FFT  shape of windowed signal in time is --------------------------------', xsubs.shape)
        ...
        print("----------------------------- Data shape after ISTFT in time is-------------------------------------------------------------- " , scaled.shape)
